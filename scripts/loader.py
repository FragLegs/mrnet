...
BASE_DATA_DIR = '/domino/datasets/local/raw'
...
class Dataset(data.Dataset):
    def __init__(self, datadirs, diagnosis, use_gpu):
        ...
        for dir in datadirs:
            for file in os.listdir(os.path.join(BASE_DATA_DIR, dir)):
...
def load_data(diagnosis, use_gpu=False):
    ...
    train_loader = data.DataLoader(
        train_dataset, batch_size=1, num_workers=2, shuffle=True
    )
    valid_loader = data.DataLoader(
        valid_dataset, batch_size=1, num_workers=2, shuffle=False
    )
