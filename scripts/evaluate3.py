...
def run_model(model, loader, train=False, optimizer=None):
    ...
    for batch in loader:
        ...
        if loader.dataset.use_gpu:
            vol = [v.cuda() for v in vol]
            label = label.cuda()
        vol = [Variable(v) for v in vol]
...
if __name__ == '__main__':
    ...
    for diagnosis in ['abnormal', 'acl', 'meniscus']:
        ...
        models_dir = f'runs/{model_name}/{diagnosis}'
        ...
        for pred, label, case in zip(*evaluate(valid_loader, model)):
            ...
            val_cases[case][f'{diagnosis}_label'] = label
            val_cases[case][f'{diagnosis}_pred'] = pred
        ...
        if test_loader is not None:
            for pred, label, case in zip(*evaluate(test_loader, model)):
                ...
                test_cases[case][f'{diagnosis}_label'] = label
                test_cases[case][f'{diagnosis}_pred'] = pred
